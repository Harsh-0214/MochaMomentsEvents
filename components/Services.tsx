        ...
        <Reveal as="ul" className="mt-16 grid gap-6 md:grid-cols-3" stagger={0.08}>
          {services.map((s) => (
            <RevealItem
              as="li"
              key={s.title}
              className="group flex flex-col rounded-2xl border border-espresso/10 bg-ivory p-8 transition-[border-color,box-shadow] duration-300 hover:border-gold/60 hover:shadow-[0_12px_40px_-12px_rgba(46,31,23,0.18)]"
            >
              <s.icon
                size={26}
                strokeWidth={1.5}
